...
export async function sendUserDetailsEmail({ to, user, tempPassword, loginUrl }) {
  const safeLoginUrl = loginUrl || 'TechSphere Portal';
  const html = baseTemplate({
    title: 'Your TechSphere Account Details',
    subtitle: 'Use the credentials below to sign in.',
    bodyHtml: `
      <div style="margin-bottom:12px;">Hi ${user?.full_name || 'there'},</div>
      <div style="margin-bottom:12px;">Your login credentials:</div>
      <ul style="padding-left:18px;margin:0 0 12px 0;color:#1a2230;">
        <li><strong>Email:</strong> ${user?.email || 'N/A'}</li>
        <li><strong>Temporary Password:</strong> ${tempPassword || 'N/A'}</li>
      </ul>
      <div style="margin-bottom:12px;">Profile details on record:</div>
      <ul style="padding-left:18px;margin:0;color:#1a2230;">
        <li><strong>Name:</strong> ${user?.full_name || 'N/A'}</li>
        <li><strong>Branch:</strong> ${user?.branch || 'N/A'}</li>
        <li><strong>Semester:</strong> ${user?.semester || 'N/A'}</li>
        ...
        <li><strong>Status:</strong> ${user?.approved ? 'Approved' : 'Pending'}</li>
      </ul>
      <div style="margin-top:14px;font-size:12px;color:#8a9ab0;">
        Sign in at ${safeLoginUrl}. For security, please change your password after logging in.
      </div>
    `
  });
  const text = [
    'Your TechSphere account details:',
    `Email: ${user?.email || 'N/A'}`,
    `Temporary Password: ${tempPassword || 'N/A'}`,
    `Name: ${user?.full_name || 'N/A'}`,
  ...
  return sendEmail({ to, subject: 'TechSphere Account Details', html, text });
}
